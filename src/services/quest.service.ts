 ...
 *
 * SECURITY: completion is determined server-side by evaluateCheckpointSubmission().
 * The client no longer sends `completed` — the act of submitting a non-empty
 * selfReflectionAnswer IS the completion signal.
 */
export const completeCheckpoint = async (
    lessonId: string,
    checkpointId: string,
    userId: string,
    submission: { selfReflectionAnswer?: string }
) => {
    ...
    const evaluation = evaluateCheckpointSubmission(submission);
    if (!evaluation.isComplete) {
        throw new AppError(400, 'يجب تقديم إجابة غير فارغة');
    }

    const xpEarned = checkpoint.xpAward;
    ...
    return {
        ...
        nextCheckpoint: nextCheckpoint
            ? {
                  ...
                  titleAr: nextCheckpoint.titleAr,
