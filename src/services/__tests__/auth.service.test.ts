...
describe('Auth Service', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        // Make $transaction invoke the callback with the mocked prisma client itself
    ...
    describe('register', () => {
        ...
        it('should create a new user and return tokens', async () => {
            ...
            (prisma.userQuest.createMany as jest.Mock).mockResolvedValue({ count: 0 });
        ...
        it('should assign daily quests to the new user', async () => {
            (prisma.user.findUnique as jest.Mock).mockResolvedValue(null);
            (prisma.user.create as jest.Mock).mockResolvedValue({
                id: 'user-uuid',
                email: validUserData.email,
            ...
            (prisma.userStats.create as jest.Mock).mockResolvedValue({});
            (prisma.quest.findMany as jest.Mock).mockResolvedValue([{ id: 'quest-1' }, { id: 'quest-2' }]);
            (prisma.userQuest.createMany as jest.Mock).mockResolvedValue({ count: 2 });
            (redis.set as jest.Mock).mockResolvedValue('OK');
            (generateAccessToken as jest.Mock).mockReturnValue('dummy-access-token');
            (generateRefreshToken as jest.Mock).mockReturnValue('dummy-refresh-token');
            ...
            expect(prisma.userQuest.createMany).toHaveBeenCalledWith({
                data: [
                    { userId: 'user-uuid', questId: 'quest-1', progress: 0, completed: false },
                    { userId: 'user-uuid', questId: 'quest-2', progress: 0, completed: false },
                ],
                skipDuplicates: true,
            });
        });
    ...
    describe('refreshToken', () => {
        it('should return new access token for valid refresh token', async () => {
            ...
            // The service now passes a clean payload (without exp/iat) to generateAccessToken
