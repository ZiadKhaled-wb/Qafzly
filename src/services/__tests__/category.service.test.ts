...
describe('Category Service', () => {
    ...
    describe('getCategoryById', () => {
        it('should return category with children and published paths', async () => {
            const mockCategory = { id: 'cat-1', name: 'Test', children: [], paths: [] };
    ...
    describe('deleteCategory', () => {
        it('should soft-delete by setting path categoryId to null, child parentId to null, and delete category', async () => {
            (prisma.courseCategory.findUnique as jest.Mock).mockResolvedValue({ id: 'cat-1' });
            (prisma.path.updateMany as jest.Mock).mockResolvedValue({ count: 2 });
            ...
            expect(prisma.path.updateMany).toHaveBeenCalledWith({
