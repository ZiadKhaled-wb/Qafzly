...
export const addChild = asyncHandler(async (req: Request, res: Response) => {
    ...
    // req.body has either { childId } or { email } — validated by addChildSchema
    const child = await parentService.addChild(parentUserId, req.body);
...
export const updateChildSettings = asyncHandler(async (req: Request, res: Response) => {
    const parentUserId = (req as any).user.userId;
    const childId = req.params.childId;
    const settings = await parentService.updateChildSettings(
        parentUserId,
        childId as string,
        req.body
    );
