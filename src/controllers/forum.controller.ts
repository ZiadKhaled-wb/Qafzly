...
export const listPosts = asyncHandler(async (req: Request, res: Response) => {
    const result = await forumService.listPosts(req.query);
...
export const updatePost = asyncHandler(async (req: Request, res: Response) => {
    const userId = (req as any).user.userId;
    const isAdmin = (req as any).user.role === 'ADMIN';
    const post = await forumService.updatePost((req.params.id as string), userId, req.body, isAdmin);
...
export const addComment = asyncHandler(async (req: Request, res: Response) => {
    const userId = (req as any).user.userId;
    const comment = await forumService.addComment((req.params.postId as string), userId, req.body);
    return apiResponse(res, 201, comment, 'تم إضافة التعليق بنجاح');
});

export const getComments = asyncHandler(async (req: Request, res: Response) => {
    const result = await forumService.getComments((req.params.postId as string), req.query);
...
export const updateComment = asyncHandler(async (req: Request, res: Response) => {
    const userId = (req as any).user.userId;
    const isAdmin = (req as any).user.role === 'ADMIN';
    const comment = await forumService.updateComment((req.params.id as string), userId, req.body, isAdmin);
...
export const markBestAnswer = asyncHandler(async (req: Request, res: Response) => {
    const userId = (req as any).user.userId;
    const post = await forumService.markBestAnswer((req.params.id as string), req.body.commentId, userId);
...
export const searchPosts = asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await forumService.searchPosts(q, pagination);
